    ...
    try {
        ...
        if (lyrics != "") {
            ...
            metadata.openGraph.images = [
                {
                    url: `https://spotify-lyrics-share.onrender.com/get-og/?lyrics=${encodeURIComponent(highlightedLyrics)}&title=${data.title}`,
                    width: 1200,
                    height: 630,
                },
            ];
...
const page = async ({ params, searchParams }) => {
    ...
    try {
        ...
        const highlightedLines = highlighted.split(',').map(Number);
        const highlightedLyrics = highlightedLines.length > 0
            ? data.lyrics
                .split('\n')
                .filter((line, index) => highlightedLines.includes(index))
                .join('\n')
        ...
        return (
            <SuspenseWrapper>
                <ResultPage
                    ...
                    highlightedLyrics={highlightedLyrics}
                />
            </SuspenseWrapper>
        );
    } catch (error) {
